 ...
 *
 * Compile with:
 *   cc -std=c11 -O2 -I c -I cpp -o prttil-main c/main.c c/ports.c c/pkgs.c cpp/s7.c
 *
 * Registers Scheme procedures (load-ports, load-pkgs, ...) that delegate to
 * the C port and pkg arrays, then hands control over to s7: it loads a
 * script if one is given on the command line, otherwise drops into an
 * interactive REPL.
 */
